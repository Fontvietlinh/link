...
async def shorten_url(request: ShortenRequest, db: Session = Depends(get_db_session)):
    ...
    existing_record = db.query(URLMapping).filter(URLMapping.long_url == target_url).first()
    if existing_record:
...
@app.get("/{short_code}")
async def redirect_to_original(short_code: str, db: Session = Depends(get_db_session)):
    ...
    raise HTTPException(status_code=404, detail="Đường dẫn rút gọn không tồn tại trên hệ thống.")
